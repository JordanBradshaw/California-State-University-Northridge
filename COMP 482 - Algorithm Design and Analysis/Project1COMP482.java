...
public class Project1COMP482 {
    ...
    public static boolean findInWomen(int VIM, int MAN) {
        for (int k = 0; k < totalPeopleInt; k++) {
            ...
            if (solution[VIM - 1] == womenPrefer[VIM - 1][k]) {
                // SOLUTION VALUE AND NUMBER IN WOMANS MATRIX ARE THE SAME THUS RETURN FALSE FOR
                // NOBREAKUP
                return false;
            } // else if (Integer.compare((MAN+1), womenPrefer[VIM-1][k]) == 0){
            else if ((MAN + 1) == womenPrefer[VIM - 1][k]) {
                // PASSED MAN AND THE NUMBER IN WOMANS MATRIX ARE THE SAME THUS HIGHER ON BOTH
                // PREFERED LISTS THUS RETURN TRUE FOR BREAKUP
                return true;
            } else {
    ...
    public static boolean findInMen(int[] solution) {// solution contains an array of the given permutation
        ...
        for (int i = 0; i < totalPeopleInt; i++) {
            ...
            outerloop: for (int j = 0; j < totalPeopleInt; j++) {// THIS IS GOING LEFT2RIGHT IN THE PREFERED MAN MATRIX
                if (menPrefer[i][j] == solution[i]) {
                    if ((findInWomen(menPrefer[i][j], i)) == true) {
                        // IF THE MAN THATS PASSED INTO THE WOMANS ARRAY IS HIT FIRST THAT MEANS THEY
                        // BOTH SHOULD BE WITH EACHOTHER THUS BREAKUP
                        return true;
                    } else {
                        // IF IT HITS THE WOMAN THATS PREFERED IN THE ARRAY FIRST THEN HIGHEST PREFERED
                        // IS SELECTED
    ...
    private static void swap(int[] input, int a, int b) {// This helps assist the previous method in creating all
                                                         // possible permutations by
        ...
        try {
            try (BufferedReader reader = new BufferedReader(new FileReader(filename))) {
                ...
                while ((line = reader.readLine()) != null) {
                    if (slot <= totalPeopleInt) {// WHEN I GET TO ARRAY SLOT WITH HOW MANY PEOPLE I HAVE THAT WILL BE
                                                 // MEN MATRIX
